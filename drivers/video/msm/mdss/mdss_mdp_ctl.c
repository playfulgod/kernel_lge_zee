...
#ifdef QMC_SLIMPORT_UNDERRUN_PATCH
       if(ctl->intf_type == 0)
              mdss_mdp_ctl_perf_commit(ctl->mdata, MDSS_MDP_PERF_UPDATE_BUS);
       else
#endif
...
#ifdef QMC_POWERONOFF_PATCH
{
	...
	pr_info("ctl_num=%d, underrun_cnt=%d\n", ctl->num, ctl->underrun_cnt);
...
#ifdef CONFIG_OLED_SUPPORT
	/*
	 * Z needs one more ref_cnt to maintain AHB_CLOCK during booting
	 * It was increased at mdss_panel_register_done
	 */
	if (decrease_ref_cnt) {
		decrease_ref_cnt = 0;
		mdss_mdp_clk_ctrl(MDP_BLOCK_POWER_OFF, false);
